...
safeTooltip(clearAllTopBtn, ['Resets EVERY input on all tabs: model ', ...
    'lists, folders, names, options, loop breaker, extract and ', ...
    'arranging fields. The logs are kept.']);
...
tab3 = uitab(tabs, 'Title', 'Arranging');
...
% block spacing used when placing new blocks (From/Goto/Unit Delay)
...
lblSpacingHint = uilabel(g1, ...
    'Text', 'points between new blocks (minimum 55)', ...
    'FontAngle', 'italic', 'FontColor', [0.4 0.4 0.4]);
...
% ---- tab 3: arranging ------------------------------------------------------
g3 = uigridlayout(tab3, [11 6]);
g3.RowHeight = {24, 30, 30, 30, 26, 26, 26, 26, 26, 36, '1x'};
g3.ColumnWidth = {150, '1x', '1x', '1x', '1x', 105};
g3.Padding = [14 10 14 10];
g3.RowSpacing = 6;
g3.ColumnSpacing = 8;

hint3 = uilabel(g3, ...
    'Text', ['1) Open the model    2) Choose the level (top or subsystem)    ' ...
    '3) Arrange'], ...
    'FontAngle', 'italic', 'FontColor', [0.4 0.4 0.4]);
hint3.Layout.Row = 1;
hint3.Layout.Column = [1 6];

lblArrModel = uilabel(g3, 'Text', 'Model:', 'FontWeight', 'bold');
lblArrModel.Layout.Row = 2;
lblArrModel.Layout.Column = 1;

arrModelEdit = uieditfield(g3, 'text', ...
    'Value', getpref('teamtools', 'GeneratedModel', ''), ...
    'Placeholder', 'e.g. test01 (filled automatically after Generate)');
arrModelEdit.Layout.Row = 2;
arrModelEdit.Layout.Column = [2 6];

lblLevel = uilabel(g3, 'Text', 'Arrange level:', 'FontWeight', 'bold');
lblLevel.Layout.Row = 3;
lblLevel.Layout.Column = 1;

levelDrop = uidropdown(g3, ...
    'Items', {'Level 1 - top level of the model', ...
              'Level 2 - inside a subsystem'}, ...
    'Value', 'Level 1 - top level of the model', ...
    'ValueChangedFcn', @arrangeLevelChanged);
safeTooltip(levelDrop, ['Level 1 arranges the root of the model ', ...
    '(root Inports, Outports, the main subsystem). Level 2 arranges ', ...
    'inside a subsystem you pick - e.g. the main subsystem, where ', ...
    'the reference models and From/Goto blocks live. Only the chosen ', ...
    'level is touched, never anything deeper.']);
levelDrop.Layout.Row = 3;
levelDrop.Layout.Column = [2 6];

lblArrSub = uilabel(g3, 'Text', 'Subsystem (Level 2):');
lblArrSub.Layout.Row = 4;
lblArrSub.Layout.Column = 1;

arrSubDrop = uidropdown(g3, 'Items', {'(pick the level and Refresh)'}, ...
    'Enable', 'off');
arrSubDrop.Layout.Row = 4;
arrSubDrop.Layout.Column = [2 5];

arrRefreshBtn = uibutton(g3, 'push', 'Text', 'Refresh', ...
    'FontSize', 11, 'Enable', 'off', ...
    'ButtonPushedFcn', @refreshArrangeSubs);
arrRefreshBtn.Layout.Row = 4;
arrRefreshBtn.Layout.Column = 6;

chkSameSize = uicheckbox(g3, ...
    'Text', 'Same size for models and From/Goto blocks (biggest)', ...
    'Value', true);
safeTooltip(chkSameSize, ['Every model-reference block gets the size of ', ...
    'the biggest one (top-left corner fixed) - the height is raised ', ...
    'if needed so ports end up evenly spaced and tag blocks never ', ...
    'overlap. All From + Goto blocks get one shared size, anchored ', ...
    'on the port side.']);
chkSameSize.Layout.Row = 5;
chkSameSize.Layout.Column = [1 6];

chkAlignPorts = uicheckbox(g3, ...
    'Text', 'Align columns: Inports left, Outports right, From/Goto straight to ports', ...
    'Value', true);
safeTooltip(chkAlignPorts, ['Inport blocks form one column on the left ', ...
    '(ordered by port number), Outport blocks one column on the ', ...
    'right, evenly spaced. From, Goto and Unit Delay blocks move to ', ...
    'the exact height of the port they serve, so every connection ', ...
    'is one straight horizontal line with no overlapping names.']);
chkAlignPorts.Layout.Row = 6;
chkAlignPorts.Layout.Column = [1 6];

chkTidyLines = uicheckbox(g3, ...
    'Text', 'Tidy line routing (orthogonal lanes, crossings allowed)', ...
    'Value', true);
safeTooltip(chkTidyLines, ['Point-to-point lines are reshaped into clean ', ...
    'right-angle routes through free lanes; feedback lines run over ', ...
    'the top. Branched signals keep automatic routing. Lines may ', ...
    'cross each other but never pass through blocks.']);
chkTidyLines.Layout.Row = 7;
chkTidyLines.Layout.Column = [1 6];

lblSpFromModel = uilabel(g3, 'Text', 'From-Model:');
lblSpFromModel.Layout.Row = 8;
lblSpFromModel.Layout.Column = 1;
gapFromModelEdit = uieditfield(g3, 'numeric', ...
    'Value', getpref('teamtools', 'ArrFromModelGap', 100), ...
    'Limits', [20 5000]);
safeTooltip(gapFromModelEdit, ['Gap between the FROM blocks and the ', ...
    'subsystem, in points (default 100). Applied by Arrange and at ', ...
    'Generate.']);
gapFromModelEdit.Layout.Row = 8;
gapFromModelEdit.Layout.Column = 2;

lblSpModelGoto = uilabel(g3, 'Text', 'Model-Goto:');
lblSpModelGoto.Layout.Row = 8;
lblSpModelGoto.Layout.Column = 3;
gapModelGotoEdit = uieditfield(g3, 'numeric', ...
    'Value', getpref('teamtools', 'ArrModelGotoGap', 100), ...
    'Limits', [20 5000]);
safeTooltip(gapModelGotoEdit, ['Gap between the subsystem and its GOTO ', ...
    'blocks, in points (default 100). Applied by Arrange and at ', ...
    'Generate.']);
gapModelGotoEdit.Layout.Row = 8;
gapModelGotoEdit.Layout.Column = 4;

lblSpFromDelay = uilabel(g3, 'Text', 'From-Delay:');
lblSpFromDelay.Layout.Row = 8;
lblSpFromDelay.Layout.Column = 5;
gapFromDelayEdit = uieditfield(g3, 'numeric', ...
    'Value', getpref('teamtools', 'ArrFromToDelayGap', 40), ...
    'Limits', [10 2000]);
safeTooltip(gapFromDelayEdit, ['Gap between a FROM block and the Unit ', ...
    'Delay behind it, in points (default 40). Applied by Arrange and ', ...
    'at Generate.']);
gapFromDelayEdit.Layout.Row = 8;
gapFromDelayEdit.Layout.Column = 6;

lblSpModelModel = uilabel(g3, 'Text', 'Model-Model:');
lblSpModelModel.Layout.Row = 9;
lblSpModelModel.Layout.Column = 1;
gapModelModelEdit = uieditfield(g3, 'numeric', ...
    'Value', getpref('teamtools', 'ArrModelToModelGap', 400), ...
    'Limits', [50 20000]);
safeTooltip(gapModelModelEdit, ['Space between one subsystem and the ', ...
    'next, in points (default 400). Used at Generate (stacked and ', ...
    'side-by-side). Arrange never moves subsystems, so it only ', ...
    're-spaces the From/Goto/delay columns.']);
gapModelModelEdit.Layout.Row = 9;
gapModelModelEdit.Layout.Column = 2;

lblSpacingHint = uilabel(g3, ...
    'Text', 'gaps in points - used by Arrange and Generate', ...
    'FontAngle', 'italic', 'FontColor', [0.4 0.4 0.4]);
lblSpacingHint.Layout.Row = 9;
lblSpacingHint.Layout.Column = [3 6];

arrangeBtn = uibutton(g3, 'push', 'Text', 'Arrange', ...
    'FontSize', 12, 'FontWeight', 'bold', ...
    'ButtonPushedFcn', @doArrange);
safeTooltip(arrangeBtn, ['Arranges the chosen level. Reference models ', ...
    'and subsystems are never moved; From/Goto/delay blocks are ', ...
    're-spaced to the gaps above; a .bak backup is made and the ', ...
    'model saves only if every safety check passes.']);
arrangeBtn.Layout.Row = 10;
arrangeBtn.Layout.Column = [1 2];

lblArrangeHint = uilabel(g3, ...
    'Text', ['models never move - sizes, port columns, gaps and ', ...
    'lines are arranged'], ...
    'FontAngle', 'italic', 'FontColor', [0.4 0.4 0.4]);
lblArrangeHint.Layout.Row = 10;
lblArrangeHint.Layout.Column = [3 6];

log3 = uitextarea(g3, 'Editable', 'off', ...
    'Value', {'Ready. Open a model, choose the level, press Arrange.'});
log3.Layout.Row = 11;
...
% --- Tab 3: arranging
arrModelEdit.Value = '';
levelDrop.Value = 'Level 1 - top level of the model';
try
    arrSubDrop.Items = {'(pick the level and Refresh)'};
catch
end
arrSubDrop.Enable = 'off';
arrRefreshBtn.Enable = 'off';
chkSameSize.Value = true;
chkAlignPorts.Value = true;
chkTidyLines.Value = true;
...
arrModelEdit.Value = result.TargetModel;
...
function arrangeLevelChanged(~, ~)
%ARRANGELEVELCHANGED Enable the subsystem picker only for Level 2.

if strcmp(levelDrop.Value, 'Level 2 - inside a subsystem')
    arrSubDrop.Enable = 'on';
    arrRefreshBtn.Enable = 'on';
    setStatus('Level 2: press Refresh and pick the subsystem to arrange.');
else
    arrSubDrop.Enable = 'off';
    arrRefreshBtn.Enable = 'off';
    setStatus('Level 1: the top level of the model will be arranged.');
end
end

function refreshArrangeSubs(~, ~)
%REFRESHARRANGESUBS List the subsystems of the open model.

modelName = char(strtrim(arrModelEdit.Value));
if isempty(modelName)
    modelName = state.LastGeneratedModel;
end
if isempty(modelName)
    setStatus('Enter the model name first.');
    return;
end
if ~isvarname(modelName)
    setStatus(sprintf(['"%s" is not a valid model name. Use the model ', ...
        'name only, e.g. "test01".'], modelName));
    return;
end
if ~bdIsLoaded(modelName)
    try
        arrSubDrop.Items = {'(model is not open)'};
    catch
    end
    setStatus(sprintf('Model "%s" is not open in Simulink.', modelName));
    return;
...
try
    ...
    logTo(log3, ['ERROR: ' errorDetails(findError)]);
    notify(app, errorDetails(findError), 'Could not list subsystems', 'error');
    return;
end
if isempty(subSystems)
    try
        arrSubDrop.Items = {'(no subsystems found)'};
    catch
    end
    setStatus('No subsystems found in this model.');
    return;
end
subSystems = sort(subSystems);
try
    arrSubDrop.Items = subSystems;
    arrSubDrop.Value = subSystems{1};
catch
end
setStatus(sprintf('%d subsystem(s) listed - pick the one to arrange.', ...
    numel(subSystems)));
logTo(log3, sprintf('Listed %d subsystem(s) in "%s".', ...
    numel(subSystems), modelName));
end

function doArrange(~, ~)
%DOARRANGE Arrange the chosen level of the open model.
...
if strcmp(levelDrop.Value, 'Level 1 - top level of the model')
    targetSystem = modelName;
else
    targetSystem = char(arrSubDrop.Value);
    if isempty(targetSystem) || ~contains(targetSystem, '/')
        setStatus('Press Refresh and pick a subsystem first (Level 2).');
        return;
    end
    try
        get_param(targetSystem, 'Handle');
    catch
        setStatus(sprintf(['Subsystem "%s" was not found - press Refresh ', ...
            'and pick from the list.'], targetSystem));
        return;
    end
end

options = struct( ...
    'SameSize',         logical(chkSameSize.Value), ...
    'AlignPortColumns', logical(chkAlignPorts.Value), ...
    'TidyLines',        logical(chkTidyLines.Value), ...
    'Spacing',          spacingGaps());

answer = confirmDialog(app, ...
    sprintf(['Arrange this level now?\n\n%s\n\n', ...
    'Reference models and subsystems are never moved - only sizes, ', ...
    'Inport/Outport columns, the From/Goto/delay gaps you set and ', ...
    'line routes change. A .bak backup is ', ...
    'made first, and the model saves only if every safety check ', ...
    'passes.'], targetSystem), ...
    'Arrange', 'Arrange', 'Cancel');
if ~answer
    return;
end

try
    result = arrangeModelLayout(targetSystem, options);
catch arrangeError
    logTo(log3, ['ERROR: ' errorDetails(arrangeError)]);
    notify(app, errorDetails(arrangeError), 'Arrange failed', 'error');
    setStatus('Arrange failed - see the log.');
    return;
end

summary = {'==== Arrange ===='};
summary{end + 1} = sprintf('Level arranged: %s', result.System);
summary{end + 1} = sprintf('Model-reference blocks resized to the biggest: %d', ...
    result.Counts.ModelsResized);
summary{end + 1} = sprintf('From/Goto blocks resized (one shared size): %d', ...
    result.Counts.FromGotoResized);
summary{end + 1} = sprintf('Inport blocks aligned into the left column: %d', ...
    result.Counts.InportsAligned);
summary{end + 1} = sprintf('Outport blocks aligned into the right column: %d', ...
    result.Counts.OutportsAligned);
summary{end + 1} = sprintf('From/Goto/delay blocks aligned straight to their ports: %d', ...
    result.Counts.TagBlocksAligned);
summary{end + 1} = sprintf('From/Goto/delay blocks re-spaced to your gaps: %d', ...
    result.Counts.TagBlocksRespaced);
summary{end + 1} = sprintf('Lines straightened: %d (left on automatic routing: %d)', ...
    result.Counts.LinesStraightened, result.Counts.LinesLeftAuto);
if isempty(result.BackupFile)
    summary{end + 1} = 'Backup: none could be written (model saved anyway).';
else
    summary{end + 1} = ['Backup: ' result.BackupFile];
end
if isempty(result.Warnings)
    summary{end + 1} = 'No warnings - everything the checks allow was applied.';
else
    summary{end + 1} = 'Warnings:';
    for warningIndex = 1:numel(result.Warnings)
        summary{end + 1} = ['  - ' result.Warnings{warningIndex}]; %#ok<AGROW>
    end
end
summary{end + 1} = 'Model saved. Update the diagram (Ctrl+D) to verify.';
logMany(log3, summary);
setStatus('Arrange finished - see the log.');
notify(app, sprintf(['%s arranged and saved.\n\n', ...
    'Sizes: %d model + %d From/Goto blocks; columns: %d Inports / ', ...
    '%d Outports; lines: %d straightened.\n', ...
    'Run the other level too if you want (Level 1 / Level 2).'], ...
    result.System, result.Counts.ModelsResized, ...
    result.Counts.FromGotoResized, result.Counts.InportsAligned, ...
    result.Counts.OutportsAligned, result.Counts.LinesStraightened), ...
    'Arrange', 'success');
end
