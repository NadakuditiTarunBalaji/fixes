...
% sample times and matching parameter source files to test teamtools.
    ...
    % Ensure Simulink engine is loaded in memory
    if ~bdIsLoaded('simulink')
        try load_system('simulink'); catch, end
    end

    fprintf('=== Setting up teamtools 10-Model Test Workspace ===\n');
    fprintf('Target Directory: %s\n\n', testDir);

    % 10 distinct models with different discrete sample rates (GCD = 0.001s)
    modelConfig = struct(...
        'Name', { ...
            'SensorHub', ...       % Model 1: Fast sensor acquisition (1ms)
            'FastFilter', ...      % Model 2: Signal processing (2ms)
            'VibeMonitor', ...     % Model 3: Engine vibration analytics (5ms)
            'EngineController', ...% Model 4: Core controller (10ms)
            'GearSelector', ...    % Model 5: Shift scheduler (10ms)
            'ThermalManager', ...  % Model 6: Cooling loops (20ms)
            'CabinComfort', ...    % Model 7: Climate controller (50ms)
            'DiagnosticsUnit', ... % Model 8: System health and logging (100ms)
            'DriverDisplay', ...   % Model 9: Display refresh (Inherited)
            'ActuatorDriver' ...   % Model 10: Low-level driver outputs (5ms)
        }, ...
        'Ts', { ...
            '0.001', ... % 1ms
            '0.002', ... % 2ms
    ...
    for i = 1:numel(modelConfig)
        m = modelConfig(i);
        fprintf('Generating Model %2d/10: %-20s (Ts = %5s)... ', i, m.Name, m.Ts);
        ...
        try
            ...
            % Add Inport & Outport using universal built-in blocks
            add_block('built-in/Inport', [m.Name '/' m.Inport], 'Position', [40 50 70 70]);
            add_block('built-in/Outport', [m.Name '/' m.Outport], 'Position', [200 50 230 70]);
    ...
    fprintf('Run "teamtools" to begin testing.\n');
end